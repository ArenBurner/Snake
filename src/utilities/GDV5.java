...
public abstract class GDV5 extends Canvas implements Runnable, KeyListener {
	...
	private static int MAX_WINDOW_X = 800*multiplier;
	private static int MAX_WINDOW_Y = 600*multiplier;
	...
	public void start() {
		...
		frame = new JFrame();
		frame.setSize(MAX_WINDOW_X, MAX_WINDOW_Y);
		frame.add(this);
		frame.pack();
	...
	public static int collisionDirection(RectangularShape stationary, RectangularShape projectile, int dx, int dy) {
		...
		int result = -1; // default intersects from right

		if (previousYPos + height <= stationary.getY() && projectile.getMaxY() >= stationary.getY()
				&& projectile.intersects((Rectangle2D) stationary)) {
			// intersects from top
			result = 1;
			...
			result = 4;
		}else if (previousYPos >= stationary.getY() + stationary.getHeight()
				&& projectile.getY() <= stationary.getY() + stationary.getHeight()
				&& projectile.intersects((Rectangle2D) stationary)) {
